  ...
  if (isLoading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-white">
        ...
        <span className="mt-3 text-xs tracking-wider text-slate-400 uppercase font-medium">Fetching active ledger...</span>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-8 max-w-md mx-auto mt-20 border border-red-100 rounded-lg text-center bg-white">
        ...
        <p className="text-xs text-slate-400 mt-1">Unable to interface with core ledger pipeline.</p>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="w-full bg-white px-6 py-10 lg:px-14 font-sans min-h-screen text-slate-900 antialiased">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-12 pb-6 border-b border-slate-100">
            <p className="text-[10px] tracking-widest text-slate-400 uppercase font-bold mb-1">Pipeline Distribution System</p>
            <h1 className="text-2xl font-semibold tracking-tight text-slate-900">In-Progress Ledger</h1>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="space-y-6">
              <div className="flex items-baseline justify-between border-b border-slate-100 pb-3">
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400">Action Required</h2>
              ...
              <div className="space-y-3">
                {needsProposal.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl">
                    <p className="text-xs text-slate-400">Pipeline empty. No actions pending.</p>
                  </div>
                ) : (
                  needsProposal.map(lead => (
                    ...
                    >
                      <div className="flex justify-between items-start gap-4 mb-1">
                        <h3 className="text-sm font-semibold text-slate-900 group-hover:text-slate-600 transition-colors">{lead.leadName}</h3>
                      ...
                      <p className="text-xs text-slate-500 mb-4 font-medium">
                        {lead.title || 'Executive'} {lead.companyName && <span>@ <span className="text-slate-800 font-semibold">{lead.companyName}</span></span>}
                      </p>
                      <div className="flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span className="truncate tracking-tight">{lead.email || '—'}</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="space-y-6">
              <div className="flex items-baseline justify-between border-b border-slate-100 pb-3">
                <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400">Awaiting Response</h2>
                <span className="text-xs font-mono font-medium text-slate-500 bg-slate-50 px-2 py-0.5 rounded border border-slate-200/60">
              ...
              <div className="space-y-3">
                {proposalSent.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl">
                    <p className="text-xs text-slate-400">No proposals currently deployed on file.</p>
                  </div>
                ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/10 backdrop-blur-xs animate-in fade-in duration-150">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 shadow-xl w-full max-w-2xl flex flex-col max-h-[85vh] relative z-10 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-start bg-white">
              <div>
                <h2 className="text-base font-semibold text-slate-900">{selectedLead.leadName}</h2>
              ...
              {!isComposing ? (
                ...
                <div className="p-6 space-y-6">
                  ...
                  {!selectedLead.proposalSent ? (
                    <div className="p-4 border border-slate-200 rounded-lg bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div>
                        <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wide">Document Missing</h4>
                        <p className="text-xs text-slate-400 mt-0.5">No proposal document configuration detected on file.</p>
                      </div>
                    ...
                    <div className="p-4 border border-slate-200 rounded-lg bg-slate-50 space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs font-bold text-slate-700 uppercase tracking-wide">Proposal Asset Sent • Awaiting Client Action</p>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Contact Ledger</h3>
                    ...
                    <div className="space-y-4">
                      <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Internal Metadata</h3>
                      <div className="space-y-3 text-xs border border-slate-100 rounded-lg p-4">
                        <div>
                          ...
                          <div className="flex items-center gap-0.5 text-[#99B562]">
                            {[1, 2, 3, 4, 5].map(star => (
                              <svg key={star} className={`w-3.5 h-3.5 ${star <= selectedLead.leadScore ? 'fill-current' : 'text-slate-200 fill-none'}`} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path></svg>
                            ))}
                          </div>
                        </div>
                        <div>
                ...
                <div className="p-6 space-y-4 border-t border-slate-100">
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        ...
                        <input type="text" disabled value={selectedLead.email || ''} className="w-full bg-slate-50 border border-slate-200 text-slate-500 rounded px-3 py-1.5 text-xs font-mono cursor-not-allowed focus:outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Subject Header</label>
                        <input type="text" value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-800 rounded px-3 py-1.5 text-xs focus:outline-none focus:border-slate-900 transition-all" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Proposal URI Resource Path</label>
                        <input type="url" value={proposalLink} onChange={(e) => setProposalLink(e.target.value)} placeholder="https://secure.ledger.path/asset" className="w-full bg-white border border-slate-200 text-slate-800 rounded px-3 py-1.5 text-xs font-mono focus:outline-none focus:border-slate-900 transition-all" />
                      </div>
                      <div>
                        ...
                        <textarea rows={5} value={emailBody} onChange={(e) => setEmailBody(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-800 rounded px-3 py-2 text-xs focus:outline-none focus:border-slate-900 transition-all resize-none font-mono" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    ...
                    >
                      {isSending ? <div className="w-3 h-3 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : 'Dispatch Pipeline Send'}
                    </button>
                  </div>
                </div>
              )}
      ...
      {selectedLead && pendingQualification && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-xs animate-in fade-in duration-100">
          <div className="absolute inset-0" onClick={() => { setPendingQualification(null); setDealDocLink(""); }}></div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Resource Registry Insertion</h3>
              <p className="text-xs text-slate-500 mt-1">
                Link absolute transaction documentation prior to switching status to <span className="font-bold text-slate-900">{pendingQualification}</span>.
              </p>
            </div>

            <div className="p-5">
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Deal Document URI</label>
              <input
                type="url"
                required
                value={dealDocLink}
            ...
            <div className="px-5 py-3 border-t border-slate-100 flex justify-end gap-2">
              <button
                onClick={() => { setPendingQualification(null); setDealDocLink(""); }}
