...
export default function Sales_My_Leads() {
  ...
  // Note Modal State (history based)
  ...
  // --- Normal Paginated Data Fetching (cursor-based pagination) ---
  const {
    data,
    fetchNextPage,
    hasNextPage,
    ...
    enabled: !!userData?._id && !isSearchMode, // ✅ সার্চ মোডে থাকলে এটা বন্ধ থাকবে
  });

  // --- Full-DB Search Query (pagination বাইপাস করে সব ডেটাতে খুঁজবে) ---
  const {
    data: searchResults,
    ...
    enabled: !!userData?._id && isSearchMode, // ✅ শুধু সার্চ থাকলে চলবে
  });

  // Flatten all fetched pages into a single leads array (normal mode)
  const leadsData: LeadData[] = useMemo(() => {
    return data?.pages.flatMap((page) => page.data) ?? [];
  }, [data]);

  // যেই ডেটাসেট আসলে দেখানো হবে — সার্চ মোড হলে সার্চ রেজাল্ট, নাহলে paginated leads
  const activeDataset: LeadData[] = isSearchMode ? (searchResults?.data ?? []) : leadsData;

  // --- Filter (status) + apply on active dataset ---
  ...
  // ✅ পুরানো note মুছবে না, নতুন note push হবে
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen bg-[#f8fafc] px-6 py-10 lg:px-14 font-sans text-slate-900 antialiased">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="mb-6 flex flex-col lg:flex-row gap-4 justify-between items-center">
            ...
            <div className="relative w-full lg:max-w-md">
              ...
              {isSearchMode && isSearchLoading && (
                <p className="absolute -bottom-5 left-0 text-[10px] text-slate-400">খুঁজছি পুরা ডাটাবেজে...</p>
              )}
            </div>
          ...
          <div className="overflow-x-auto bg-white border border-slate-200 shadow-sm rounded-lg">
            <table className="min-w-full text-sm text-left whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {!isSearchLoading && processedLeads.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-5 py-10 text-center text-slate-400 text-xs">
                      {isSearchMode ? "কোনো লিড খুঁজে পাওয়া যায়নি।" : "No records matched your search parameters."}
                    </td>
                  </tr>
                )}
            ...
            {/* --- LOAD MORE (শুধু normal mode-এ দেখাবে, সার্চ মোডে pagination নাই কারণ সব রেজাল্ট একসাথে আসে) --- */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase text-slate-400 font-bold mb-1">Indications / Notes</p>
                      <p className="text-sm text-slate-600 italic bg-white border border-slate-200 p-2 rounded mt-1">
                        {selectedLeadDetails.indications || "No descriptive entries."}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="px-6 py-5 flex justify-between items-start border-b border-slate-100 bg-slate-50/50">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Configuring calendar parameters for <span className="font-bold text-slate-800">{meetingLead.leadName}</span>.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleMeetingSubmit} className="flex-1 overflow-y-auto" noValidate>
              <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-4 space-y-4">
                  <div className="border border-slate-200 rounded-lg p-4 bg-white">
                    ...
                    <div className="space-y-3 text-xs">
                      <p>
                        <span className="text-slate-400 font-bold block mb-0.5">Enterprise</span>{" "}
                        <span className="font-medium text-slate-800">{meetingLead.companyName || "—"}</span>
                      </p>
                      <p>
                        <span className="text-slate-400 font-bold block mb-0.5">Title</span>{" "}
                        <span className="font-medium text-slate-800">{meetingLead.title || "—"}</span>
                      </p>
                      <p>
                        <span className="text-slate-400 font-bold block mb-0.5">Email</span>{" "}
                        <span className="font-mono text-slate-800">{meetingLead.email || "—"}</span>
                      </p>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-8 space-y-4">
                  {meetingError && (
                    <div className="border border-red-200 bg-red-50 px-4 py-2 text-xs font-semibold text-red-600 rounded">{meetingError}</div>
                  )}
              ...
              <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 sticky bottom-0">
                ...
                <button
                  type="submit"
                  className={`px-5 py-2 rounded text-xs font-bold text-white transition-colors shadow-xs ${
                    isMeetingConflict ? "bg-amber-500 hover:bg-amber-600" : "bg-slate-900 hover:bg-slate-800"
                  }`}
                >
      ...
      {/* --- NOTE MODAL (History + Add, ভেজাল ছাড়া) --- */}
      {noteLead && (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl relative z-10 overflow-hidden border border-slate-200 max-h-[85vh] flex flex-col">
            <div className="px-6 py-5 flex justify-between items-start border-b border-slate-100 bg-slate-50/50">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  <span className="font-bold text-slate-800">{noteLead.leadName}</span> এর সাথে এখন পর্যন্ত যত কথা হয়েছে।
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3 bg-slate-50/40">
              {!noteLead.indicationsHistory || noteLead.indicationsHistory.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-8">এখনো কোনো ফলোআপ নোট যোগ করা হয়নি।</p>
              ) : (
            ...
            <form onSubmit={handleAddNote} className="p-4 border-t border-slate-100 bg-white flex items-end gap-3">
              <div className="flex-1">
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="আজকে কি কথা হলো লিখুন..."
              ...
              >
                {mutationAddNote.isPending ? "যোগ হচ্ছে..." : "নোট যোগ করুন"}
              </button>
            </form>
          </div>
        </div>
