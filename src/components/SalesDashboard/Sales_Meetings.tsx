...
export default function Sales_Meetings() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.clientPhone?.trim()) {
      setFormError("Phone number is required to schedule a meeting.");
      return;
    }
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    if (!editFormData.clientPhone?.trim()) {
      setEditFormError("Phone number is required to update this meeting.");
      return;
    }

    setEditFormError(null);
    mutatuionUpdatemeeting.mutate({
      id: editingMeetingId,
      updatedData: { ...editFormData, clientPhone: editFormData.clientPhone.trim() },
    });
    closeEditModal();
  };
                ...
                {filteredMeetings.map((meeting) => (
                  <div key={meeting._id ?? meeting.id} className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col transition-all hover:border-[#99B562]/40 hover:shadow-lg hover:shadow-slate-200/60 hover:-translate-y-0.5 group">
                    ...
                    <div className="flex-1">
                      ...
                      {meeting.clientPhone && (
                        <div className="flex items-center gap-2 text-xs text-slate-500 mb-5 font-mono">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-3xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
               {formError && (
                 <div className="sm:col-span-2 border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-600 rounded-xl">
                   {formError}
                 </div>
               )}
               ...
               <div>
                 ...
                 <input
                   ...
                   className="w-full px-4 py-3 border border-slate-300 rounded-xl text-base text-slate-900 font-mono focus:outline-none focus:border-[#99B562] focus:ring-2 focus:ring-[#99B562]/20 transition-all"
                 />
               </div>
