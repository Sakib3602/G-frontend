...
const TaskAD = () => {
    ...
    const handleDone = (id: string) => {
        Swal.fire({
            ...
            confirmButtonColor: "#22c55e", // Green color for confirm
            cancelButtonColor: "#d33",
            confirmButtonText: "Yes, Done!"
        }).then((result) => {
            if (result.isConfirmed) {
                // এখানে API কল করে টাস্ক আপডেট করতে পারেন
    ...
    // সাবমিটের জন্য Validation এবং Success SweetAlert
    const handleSubmitWork = () => {
        ...
        // এখানে API কল করে ডেটা ডাটাবেসে পাঠাতে পারেন
        console.log("Submitting work for Task ID:", selectedTaskId, "URL:", workUrl);
        
        // সফল সাবমিট অ্যালার্ট
        Swal.fire({
            title: "Submitted!",
            text: "Your work URL has been submitted successfully.",
            icon: "success",
                    ...
                    <div key={task._id} className="border p-5 rounded-lg shadow-sm bg-white flex flex-col justify-between">
                        <div>
                            ...
                            <p className="text-xs text-gray-500">
                                Priority: <span className={`font-medium ${task.priority === 'High' ? 'text-red-600' : 'text-blue-600'}`}>{task.priority}</span>
                            </p>
                        </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">
                    <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md transform scale-100 opacity-100 transition-transform duration-300">
                        ...
                        <p className="text-sm text-gray-600 mb-6">Please provide the URL where the work is complete for verification.</p>
                        <input 
                            type="url" 
